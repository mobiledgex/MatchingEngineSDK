...
class ConnectionTests: XCTestCase {
    ...
    func testRegisterAndFindHTTPConnection() {
        ...
        let replyPromise = matchingEngine.registerAndFindHTTPConnection(devName: "franklin-mobiledgex", appName: "ios_connection_test", appVers: "1.0", carrierName: "TDG", location: loc)
        .then { request -> Promise<URLResponse> in
            let promiseInputs: Promise<URLResponse> = Promise<URLResponse>.pending()
