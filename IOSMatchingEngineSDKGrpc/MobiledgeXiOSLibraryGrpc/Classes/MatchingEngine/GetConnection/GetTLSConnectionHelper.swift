...
extension MobiledgeXiOSLibraryGrpc.MatchingEngine {
    ...
    @available(iOS 13.0, *)
    func getTCPTLSConnection(host: String, port: UInt16, netInterfaceType: String? = nil, localEndpoint: String? = nil, timeout: Double) -> Promise<NWConnection>
    {
        let promise = Promise<NWConnection>(on: .global(qos: .background)) { fulfill, reject in
            
            // find clientIP if needed
            var clientIP: String?
            var localNWEndpoint: NWEndpoint?
            do {
                ...
                if clientIP != nil {
                    localNWEndpoint = NWEndpoint.hostPort(host: NWEndpoint.Host(clientIP!), port: NWEndpoint.Port(String(describing: port))!)
                }
            } catch {
                reject(error)
                return
            ...
            if localNWEndpoint != nil {
                parameters.requiredLocalEndpoint = localNWEndpoint
            }
    ...
    @available(iOS 13.0, *)
    func getUDPDTLSConnection(host: String, port: UInt16, netInterfaceType: String? = nil, localEndpoint: String? = nil, timeout: Double) -> Promise<NWConnection>
    {
        let promise = Promise<NWConnection>(on: .global(qos: .background)) { fulfill, reject in
