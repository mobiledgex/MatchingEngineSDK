...
extension MobiledgeXiOSLibraryGrpc.PerformanceMetrics {
    ...
    public class NetTest {
        ...
        public func ping(site: Site) -> Promise<Site> {
            ...
            SPLPing.pingOnce(site.host!, configuration: configuration) { (response: SPLPingResponse) in
                ...
                if response.error != nil {
                    print("error in ping \(response.error)")
                }
                print("ping time \(response.duration * 1000)")
                let elapsedTime = after.uptimeNanoseconds - before.uptimeNanoseconds
                print("elapsed time \(Double(elapsedTime) * self.NANO_TO_MILLI)")
                site.addSample(sample: response.duration * 1000)
        ...
        public func connectAndDisconnectSocket(site: Site) -> Promise<Site> {
            ...
            return bindAndConnectSocket(site: site, addrInfo: &addrInfo, localIP: localIP)
        }
        
        // Same function is in GetBSDSocketHelper (Create socket class outside of MatchingEngine?)
        private func bindAndConnectSocket(site: Site, addrInfo: UnsafeMutablePointer<addrinfo>, localIP: String) -> Promise<Site> {
            return Promise<Site>(on: netTestDispatchQueue!) { fulfill, reject in
                ...
                let elapsedTime = after.uptimeNanoseconds - before.uptimeNanoseconds
                print("elapsed time is \(Double(elapsedTime) * self.NANO_TO_MILLI)")
                site.addSample(sample: Double(elapsedTime) * self.NANO_TO_MILLI) // convert to milliseconds
                fulfill(site)
            }
        }
